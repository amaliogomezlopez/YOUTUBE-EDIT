 ...
 * se devuelve un punto focal normalizado listo para recortar a 9:16.
 */
export async function trackFace(videoFile, media, options = {}) {
  ...
  const workspace = path.join(TMP_DIR, 'shorts-face', `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
