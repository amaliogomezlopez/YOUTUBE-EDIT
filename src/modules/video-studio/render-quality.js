import {analyzeAudioChannels} from './audio-channels.js';
import {ffprobe} from '../../lib/ffmpeg.js';
import {run} from '../../lib/utils.js';

export const VOICE_LOUDNESS = 'loudnorm=I=-14:TP=-1.5:LRA=11';
export async function finalizeShortAudio(input, output, {signal, duration, overwrite = true} = {}) {
  // Solo se recodifica la mezcla final: el video conserva sus pixeles.
  await run('ffmpeg', [overwrite ? '-y' : '-n','-i',input,...(Number.isFinite(duration)?['-t',String(duration)]:[]),'-map','0:v:0','-map','0:a:0','-c:v','copy','-af',VOICE_LOUDNESS,
...
export async function verifyShortMedia(file, {duration, signal} = {}) {
  ...
  const audioChannels = await analyzeAudioChannels(file,{signal});
  if (audioChannels.channels===2 && audioChannels.imbalanceDb>18) errors.push('Desequilibrio entre canales: '+audioChannels.imbalanceDb.toFixed(1)+' dB; revisar la voz antes de mezclar');
  ...
  return {passed: !errors.length, duration:media.duration, width:media.width, height:media.height, audioChannels, loudness, errors, warnings,
