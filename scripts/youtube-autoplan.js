...
const USAGE = `Uso:
  youtube-autoplan plan --project DIR --output DIR [--profile JSON] [--exclude ID]... [--intents JSON | --llm]
...
try {
  const {values: v, positionals: p} = parseArgs({allowPositionals: true, options: {
    ...
    intents: {type: 'string'}, llm: {type: 'boolean'}, plan: {type: 'string'}, edit: {type: 'string'}}});
  if (p[0] === 'plan') {
    ...
    const plan = planEdit({clips, profile, kit, intents});
    if (intents.warning) plan.warnings.push(intents.warning);
    const renderPlan = compileEditPlan(plan, {clips, kit});
    await mkdir(v.output, {recursive: true});
    for (const [name, value] of [['edit-plan.json', plan], ['render-plan.json', renderPlan], ['kit.json', kit], ['intents.json', intents]]) {
    ...
    console.log(JSON.stringify({output: path.resolve(v.output), duration: plan.duration, segments: plan.segments.length, decisions: counts, warnings: plan.warnings}, null, 2));
